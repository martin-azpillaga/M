...
~~~ {.set .pong .main.m}
body {mesh, material, mass, friction, restitution}

paddle : body {extents, baseVelocity, velocityInput, velocityInputRange}
ball : body {radius, serveTag, angle, length, respawnPosition, scoreApplier, collisions}
goal : body {trigger, respawnApplier, collisions, worth}
wall : body {extents}

board {number, maxNumber, points}

rules {startCount, maxCount, level}
...
serve
{
    ∀ a | serveTag
    {
        myAngle = random(a.angle)
        myLength = random(a.length)
