...
import m.m.Real2
import m.m.Value
import m.scoping.EngineComponent
import org.eclipse.xtext.generator.IFileSystemAccess2

import static m.generator.Folder.*
import static m.generator.UnityComponentType.*
import m.m.TriggerType
import m.m.AxisType
import m.m.VectorType
import org.eclipse.emf.common.util.EList
import m.m.Control
import org.eclipse.xtext.EcoreUtil2
import m.m.Loop
import m.m.Access
import m.m.ComponentAssignment
import m.m.SubrutineCall
import m.m.EngineVoidType
import m.scoping.EngineVoid
import m.m.Call
import m.scoping.EngineTransformation
...
class HybridUnity
{
	...
	def represent(Game game, IFileSystemAccess2 fileSystem) 
	{
		this.game = game
	...
	def path(TriggerType type)
	{
		switch type
		{
			...
			case D: '<Keyboard>/d'
			case E: '<Keyboard>/e'
			case EIGHT: '<Keyboard>/8'
			case F: '<Keyboard>/f'
			case FIVE: '<Keyboard>/5'
			case FOUR: '<Keyboard>/4'
			case G: '<Keyboard>/g'
			case H: '<Keyboard>/h'
			case I: '<Keyboard>/i'
			case J: '<Keyboard>/j'
			...
			case R: '<Keyboard>/r'
			case S: '<Keyboard>/s'
			case SEVEN: '<Keyboard>/7'
			case SIX: '<Keyboard>/6'
			case T: '<Keyboard>/t'
			case THREE: '<Keyboard>/3'
			case TWO: '<Keyboard>/2'
			case U: '<Keyboard>/u'
			case V: '<Keyboard>/v'
			case W: '<Keyboard>/w'
	...
	def systems()
	{
		for (system : game.systems.filter(m.m.System))
		{	
